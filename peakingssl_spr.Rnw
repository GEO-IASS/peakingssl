...
<<define-theme, echo=FALSE, warning=FALSE>>=
...
theme_simple <- 
  ...
  theme(panel.border=element_rect(size = 0,color=NA,fill=NA)) +
...
res %>% 
  ...
  scale_color_manual(name="",values=c("Base"="grey","Supervised"="black","Semi-supervised"="red")) +
  ylab("Error") +
  xlab("Number of training objects") +
  theme_simple +
...
<<peaking-asymptotic, echo=FALSE, warning=FALSE, fig.height=5.5, fig.cap="Empirical learning curves and their asymptotic approximations for different classifiers. (a) Supervised learning curve corresponding to the formulation in Eq.~\\eqref{eq:Wformulation}. (b) Supervised and semi-supervised learning curves corresponding to the formulations in Eqs.~\\eqref{eq:Tformulation}, \\eqref{eq:solutionsupervised} and \\eqref{eq:solutionsemisupervised}. Semi-supervised uses $5$ labeled objects per class and the rest as unlabeled objects.", fig.subcap=c("Within Scatter","Total Scatter"), out.width='.49\\linewidth'>>=
library(dplyr, warn.conflicts = FALSE)
library(ggplot2, warn.conflicts = FALSE)
...
out2 %>% 
  ...
  mutate(name= factor(name,c("Continued","Total","Semi-supervised","Continued Semi"),c("Supervised","Supervised","Semi-supervised","Semi-supervised"))) %>% 
  ...
  scale_x_continuous(breaks=seq(0,80,by=10),expand=c(0,0),limits=c(0,80)) +
  scale_y_continuous(breaks=seq(0,0.5,by=0.1),expand=c(0,0),limits=c(0,0.5)) +
  scale_color_manual(name="",values=c("Supervised"="black","Semi-supervised"="red"))
@
